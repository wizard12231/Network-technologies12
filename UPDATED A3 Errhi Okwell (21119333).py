...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            configure_device(your_ip_address, your_username, your_password, your_enable_password, commands)
            ...
            your_username = ssh_username  
            your_password = ssh_password  
            your_enable_password = 'class123!'  # Updated enable password
            commands = [
                "enable",
                f"{your_enable_password}",
                "configure terminal",
                # Standard ACL example (numbered 10)
                "access-list 10 permit 192.168.1.0 0.0.0.255",  # Permit traffic from 192.168.1.0/24
                "access-list 10 deny any",  # Deny all other traffic

                # Extended ACL example (numbered 100)
                "access-list 100 permit tcp 10.0.0.0 0.255.255.255 any eq 80",  # Permit HTTP from 10.0.0.0/16 to any destination
                "access-list 100 deny ip any any",  # Deny all other IP traffic

                # Apply the ACL to an interface
                "interface GigabitEthernet0/0",  # Example interface, modify as needed
                "ip access-group 10 in",  # Apply standard ACL in inbound direction
                "exit",
                
                "interface GigabitEthernet0/1",  # Another example interface, modify as needed
                "ip access-group 100 out",  # Apply extended ACL in outbound direction
                "exit",
